import { siGithub, siTiktok, siX, siYoutube } from 'simple-icons';

/** LinkedIn is absent from simple-icons, so its "in" mark is inlined. */
const LINKEDIN_PATH =
...
const PATHS: Record<string, { title: string; path: string }> = {
  ...
  x: { title: siX.title, path: siX.path },
  linkedin: { title: 'LinkedIn', path: LINKEDIN_PATH },
  youtube: { title: siYoutube.title, path: siYoutube.path },
  tiktok: { title: siTiktok.title, path: siTiktok.path },
};

/** The brand mark for a Channel, monochrome, inheriting the text colour. */
